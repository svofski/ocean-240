...
def c233toRGB(c):
    return [int(red(c) * 36.5), int(green(c) * 36.5), blue(c) * 85]

def red(x): return x&7

def green(x): return (x>>3)&7

def blue(x): return  (x>>6)&3

def colordist(c1, c2):
    return sqrt(reduce(lambda x,y: x+y, [(x1-x2)*(x1-x2) for x1, x2 in zip(c1, c2)]))


def rgba2uint32(rgba):
    rt,gt,bt,a = rgba
    return (a << 24) | (rt << 16) | (gt << 8) | bt;
